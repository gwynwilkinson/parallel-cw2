...
########################################################################################
##                                                                                    ##
##  Brute Force Encryption Breaker - OpenMP Version 1.0 by Gwyn Wilkinson 01/12/2018  ##
##                                                                                    ##
...
int main (int argc, char* argv[])
{
  ...
  /*This lets us take a command line argument specifying
    the number of threads we want to spawn for this
    task */
  if(argc == 2){
    num_threads = strtol(argv[1], &ptr, 10);

    if(num_threads>0 && num_threads<17){
      printf("%ld threads\n",num_threads);
    }else{
      printf("Invalid command-line argument. Please use a number between 1 and 16");
      return 0;
    }
  }
  ...
  /*Set the number of threads based on what
    read in at command line (default 1) */
  omp_set_num_threads(num_threads);

  /*OpenMP Pragma statement: Creates private
    copies of keyB and ciphertextB, adds up
    numkeys across all threads, and sets
    scheduler to guided */
  #pragma omp parallel for firstprivate(keyB,ciphertextB) schedule(guided) reduction(+:numkeys)
